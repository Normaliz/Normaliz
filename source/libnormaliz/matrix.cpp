...
// returns empty vector if simplex cannot be subdivided with smaller detsum
    ...
    Gred=LLL_red_transpose(Trans,Trans_inv);
    ...
    for(int i=from;i<to;++i){
        ...
        for(int j=0;j<i;++j){
            ...
            for(size_t k=0;k<dim;++k){
                ...
                sp+=fact*B[j][k];// elem[i][k].get_d()*B[j][k];
            }
...
// Lred=T*this, Tinv=inverse(T)
    ...
    Lred.GramSchmidt(G,M,0,1);
    
    int i=1;
    while(true){
        
        for(int j=i-1;j>=0;--j){
            Integer fact; // =round(M[i][j]);
    ...
    if(!using_float<Integer>()){
        Matrix<Integer> Test=T.multiplication(Tinv);
        assert(Test.equal(Matrix<Integer>(nr)));
...
// returns Lred=this*T, Tinv=inverse(T)    
    ...
    Matrix<Integer> Lred=red_trans.transpose();
    T=T_trans.transpose();
...
Matrix<nmz_float> Matrix<nmz_float>::LLL() const{
    ...
    Matrix<nmz_float> T,Tinv;
    
    return LLL_red(T,Tinv);
}*/
