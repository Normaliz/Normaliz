...
Matrix<Integer> Matrix<Integer>::LLL() const{
    ...
    // %%%%%%%%%%%%%%%%%%%%%%%
    Matrix<mpz_class> this_mpz;
    convert(this_mpz,*this);
    Matrix<mpz_class> mpz_red;
    LLL_red(this_mpz,mpz_red);
    Matrix<Integer> Lred;
    convert(Lred,mpz_red);
    // %%%%%%%%%%%%%%%%%%%%%%
    return Lred;
}

template<typename Integer>
void Matrix<Integer>::LLL_transform(Matrix<Integer>& Lred, Matrix<Integer>& T, Matrix<Integer>& Tinv) const{
// Lred =LLL_reduced(L) (sublattice generated by the rows!)
// Lred=T*this, Tinv=inverse(T)
    ...
    cout << "============" << endl;*/
    Lred=LLL();
    Matrix<Integer> sq(*this);
    sq.resize_columns(r);
    Matrix<Integer> sq_red(Lred);    
    Integer dummy;
    Matrix<Integer> I=sq.invert(dummy);
    T=sq_red.multiplication(I);
    T.scalar_division(dummy);
    Tinv=T.invert(dummy);;

    /*T.multiplication(Tinv).pretty_print(cout);
    cout << "$$$$$$$$$$$$$$" << endl;*/
}

template<typename Integer>
void Matrix<Integer>::LLL_transform_transpose(Matrix<Integer>& Lred, Matrix<Integer>& T, Matrix<Integer>& Tinv) const{
// Lred=this*T, Tinv=inverse(T)    
    
    Matrix<Integer> this_trans=transpose();
    Matrix<Integer> red_trans,T_trans, Tinv_trans;
    this_trans.LLL_transform(red_trans,T_trans,Tinv_trans);
    Lred=red_trans.transpose();
    T=T_trans.transpose();
