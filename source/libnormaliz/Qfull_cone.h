...
public:
    ...
    QuFull_Cone<Number>* Top_Cone; // reference to cone on top level
    vector<key_t> Top_Key;        // indices of generators w.r.t Top_Cone
    QuFull_Cone<Number>* Mother;   // reference to the mother of the pyramid
 ...
 */
    ...
    bool contains(const QuFull_Cone& C);
 ...
 *---------------------------------------------------------------------------
 */
    QuFull_Cone(const Matrix<Number>& M, bool do_make_prime=true);            //main constructor

    QuFull_Cone(QuFull_Cone<Number>& C, const vector<key_t>& Key); // for pyramids
