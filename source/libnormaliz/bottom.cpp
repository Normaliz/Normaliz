...
vector<Integer> best_point(const list<vector<Integer> >& bottom_candidates, const Matrix<Integer>& gens,
                           const Matrix<Integer>& SuppHyp, const vector<Integer>& grading);
...
void bottom_points_inner(const list<vector<Integer> >& bottom_candidates, SCIP* scip, 
                         Matrix<Integer>& gens, list< vector<Integer> >& new_points, 
                         vector< Matrix<Integer> >& q_gens,vector< Matrix<Integer> >& big_simplices,bool first_round);
...
template<typename Integer>
void bottom_points(list< vector<Integer> >& new_points, Matrix<Integer> gens,const vector<Integer>& grading_,
                   long recursion_depth, Integer VolumeBound) {
...
#ifdef NMZ_SCIP
    if(verbose){
            if (bottom_candidates.empty() && recursion_depth==0){
                verboseOutput() << "Computing bottom points using SCIP." << endl;
            } else{
                verboseOutput() << "Computing bottom points using projection" << endl;
            }
    }
#else
    if(verbose){
...
*/
    ...
    if (recursion_depth>1 && bottom_candidates.size()==0){
        if(verbose){
                verboseOutput() << "No subdivision points found." << endl;
        }
        return;
    }
...
//---------------------------- begin stellar subdivision -------------------
    ...
    while (!q_gens.empty()) {
        ...
        if(skip_remaining) continue;
            ...
            try {
#endif
            bottom_points_inner(bottom_candidates, scip, q_gens[i], local_new_points,
                                local_q_gens, big_simplices, recursion_depth==0);
#ifndef NCATCH
...
//---------------------------- end stellar subdivision -------------------
    ...
    // if we still have big_simplices we try again

    int counter=0;
    //cout << "A big simplex!" << endl;
    //if (!big_simplices.empty()) big_simplices.front().pretty_print(cout);
    if (recursion_depth==0 && !big_simplices.empty()){
        if(verbose){
            verboseOutput() << "There are " << big_simplices.size() << " big simplices (vol>1000*BOUND) remaining. We try subdividing them again." << endl;
        }
        for (auto it = big_simplices.begin(); it != big_simplices.end(); ++it){
            ...
            if(verbose){
                verboseOutput() << "<Trying subdivision of rrremaining big simlicial done " << it-big_simplices.begin()+1 << " / "<< big_simplices.size() << " (recursion depth " << (recursion_depth+1) << ")"  << endl;
            }
            ...
            bottom_points(new_points_again,gens,ApproxCone.Grading,recursion_depth+1,VolumeBound);
    ...
    } // remaining big simplices
    
    
    /* if (!big_simplices.empty() && first_round && 1<max_app_lvl && verbose){
            verboseOutput() << "In " << counter << " of " << big_simplices.size() 
                      << " cases the new approximation was successfull." << endl;
    }*/
    ...
    new_points.unique();
    if(verbose){
        if (recursion_depth>0) verboseOutput() << new_points.size() << " additional bottom points accumulated. " << endl;
        if (recursion_depth==0) verboseOutput() << new_points.size() << " bottom points accumulated in total." << endl;
        if (recursion_depth==0) verboseOutput() << "The sum of determinants of the stellar subdivision is " << stellar_det_sum << endl;
    }
}
...
void bottom_points_inner(const list<vector<Integer> >& bottom_candidates, SCIP* scip,
    ...
    if (bottom_candidates.empty() && first_round){ //the case we really want to use SCIP
...
vector<Integer> best_point(const list<vector<Integer> >& bottom_candidates, const Matrix<Integer>& gens,
    ...
    for (auto it = bottom_candidates.begin(); it != bottom_candidates.end(); ++it) {
        ...
        in_hyp=0;
        for (i=0; i<dim; ++i) {
            Integer test =v_scalar_product(SuppHyp[i],*it);
            if (test < 0) {
                break;
            }
            if (test == 0){
...
template void bottom_points(list< vector<long> >& new_points, Matrix<long> gens,
                            const vector<long>& grading,long recursion_depth,long VolumeBound);
#endif // NMZ_MIC_OFFLOAD
template void bottom_points(list< vector<long long> >& new_points, Matrix<long long> gens,
                            const vector<long long>& grading,long recursion_depth,long long VolumeBound);
template void bottom_points(list< vector<mpz_class> >& new_points, Matrix<mpz_class> gens,
                            const vector<mpz_class>& grading, long recursion_depth,mpz_class VolumeBound);
