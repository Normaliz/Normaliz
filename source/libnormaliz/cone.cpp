...
void Cone<renf_elem_class>::project_and_lift(const ConeProperties& ToCompute,
                                             ...
                                             const OurPolynomialSystem<renf_elem_class>& PolyInequs) {   // no primitive version yet for renf
...
void Cone<renf_elem_class>::compute_lattice_points_in_polytope(ConeProperties& ToCompute) {
    ...
    if (inhomogeneous && (ExtremeRaysRecCone.nr_of_rows() > 0 || BasisMaxSubspace.nr_of_rows() > 0 )) {
        throw BadInputException("Lattice points not computable for unbounded polyhedra over number fields");
    }
    ...
    // In this version, the lattice points are transferred into the cone
...
void Cone<renf_elem_class>::prepare_volume_computation(ConeProperties& ToCompute) {
    ...
    if (!inhomogeneous && Grading.size() != dim)  // we cannot check the cone property Grading yet
        ...
        throw NotComputableException("Volume needs an explicit grading for algebraic polytopes in the homogeneous case");
    if (getRank() != dim)
...
ConeProperties Cone<Integer>::compute(ConeProperties ToCompute) {
    ...
    // We want to make sure that the excluded faces are shown in the output/can be  returned
    ...
    */
    ...
    if (ToCompute.test(ConeProperty::Deg1Elements) && isComputed(ConeProperty::Grading)) {
        ...
        if (nr_computed_at_start == is_Computed.count()) {  // Prevention of infinite loop
...
void Cone<Integer>::try_approximation_or_projection(ConeProperties& ToCompute) {
    ...
    // data prepared, now the computation
