...
Matrix<Integer> Cone_Dual_Mode<Integer>::cut_with_halfspace(const size_t& hyp_counter, const Matrix<Integer>& Basis_Max_Subspace){
    ...
    if (rank_subspace!=0) {
        ...
        if (i!=rank_subspace) {    // the new hyperplane does not contain the intersection of the previous hyperplanes
            ...
            Matrix<Integer> Ker=M.kernel();     // the kernel of the restriction in the coordinates of Max_subspace
            New_Basis_Max_Subspace=Ker.multiplication(Basis_Max_Subspace); // now in the coordinates of the ambient space
            
            old_lin_subspace_half=Basis_Max_Subspace.VxM(restriction);  // linear form interpreted as normal vector of its kernel
                                                                        // already in the coordinates of the ambient space
