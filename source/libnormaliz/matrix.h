...
//  LLL (with transformation matrices)
    
    // returns LLL_reduced(this), (sublattice generated by the rows!), this must have rank=nr_of_rows
    Matrix<Integer> LLL() const;
    
    // Lred =LLL_reduced(L) 
    // Lred=T*this, Tinv=inverse(T)
    void LLL_transform(Matrix<Integer>& Lred, Matrix<Integer>& T, Matrix<Integer>& Tinv) const;
    
    // applies LLL to the transpose
    // this must be a square matrix, Lred=this*T, Tinv=inverse(T)
    void LLL_transform_transpose(Matrix<Integer>& Lred, Matrix<Integer>& T, Matrix<Integer>& Tinv) const;
