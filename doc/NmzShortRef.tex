...
\SubSectStart{Major algorithmic variants}

For several computation goals there exist morev than a single algorithm in  Normaliz. The program tries to choose the best variant, but it sometimes needs help by the user.
...
\begin{itemize}
	...
	\itemtt[Projection, -j] chooses project-and-lift for lattice points in polytopes (default).
...
\SubSectStart{Minor algorithmic variants}

\begin{itemize}
	\itemtt[ProjectionFloat, -J], project-and-lift with floating point arithmetic.
	
	\itemtt[NoLLL] blocks the use of LLL reduced coordinates for progect-and-lift.
	...
	\itemtt[DistributedComp] askes for distributed computation of volumes by signed decomposition.
	
	\itemtt[FixedPrecision] sets fixed precision for voume computation sby signed decomposition.
	
	\itemtt[NoSubdivision] forbids the subdivision of large simplices in the primal algorithm.
	...
	\itemtt[GradingIsPositive] tells Normaliz that there is no need to check the grading for positivity.
\end{itemize}

\SubSectStart{Integer type}

Normaliz chooses the integer type for computations automatically. But there can ve reasons for the user to fix it:
