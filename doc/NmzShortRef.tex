...
Input types can be mixed with some obvious restrictions. Roughly speaking, Normaliz forms the cone defined by the generators and intersects it with the cone defined by the constraints. The same applies to lattices. See manual for a more precise description. 
...
\begin{itemize}
	...
	$$
	 represented by the input vectors $(\xi_1,\dots,\xi_d,-\eta)$.\example{truncated\_dodecahedron\_dual}
...
Examples
\begin{Verbatim}
x[1] + 2x[3] = 5;    x[1] >= x[2] + 6;     x[1] + 2x[2] ~ 5 - 3x[]3] (12); 
\end{Verbatim}
...
Precomputed types are used for the recycling of data from previous  computations, namely extreme rays and support hyperplanes and the coordinate transformations represented by the generated sublattice (or subspace) and the maximal subspace contained in  the cone. The latter are only required if they are different from the default values $\ZZ^d$ (or $\RR^d$) and $\{0\}$, respectively.
...
\begin{itemize}
	...
	\itemtt[lattice\_ideal] is a matrix with $d$ columns containing the generators of the lattice ideal in the Laurent polynomial ring. 
...
Most computation goals include \verb|Sublattice| and \verb|SupportHyperplanes|, and there are many implications between them. 
...
\begin{arab}
\item Hilbert basis and Hilbert series in rational computations (plus class group in the homogeneous case);
\item only support hyperplanes and extreme rays for algebraic polyhedra.
\end{arab}
...
\begin{itemize}
	\itemtt[IsGorenstein, -G]: is the monoid of lattice points Gorenstein? In addition to answering this question, Normaliz also computes the generator of the interior of the monoid (the canonical module) if the monoid is Gorenstein.\example{5x5Gorenstein}
\end{itemize}
...
\begin{itemize}
	\itemtt[FVector] computes  f-vector of a polyhedron.\example{icosahedron\_prec}
...
\begin{itemize}
	\itemtt[Automorphisms] computes the integral automorphisms of rational polyhedra and the algebraic automorphisms of algebraic polytopes.\example{pythagoras\_int}
...
\begin{itemize}
	\itemtt[Sublattice, -S] (upper case S) asks Normaliz to compute the coordinate transformation to and from the efficient sublattice.\bigskip
	...
	\itemtt[ExtremeRaysFloat] does the same for the extreme rays.\bigskip
	
	\itemtt [WitnessNotIntegrallyClosed] Normaliz stops the Hilbert basis computation as soon it has found a witness confirming that the original monoid is not integrally closed.\bigskip

	\itemtt [ClassGroup, -C] is self explanatory, includes \verb|SupportHyperplanes|. Not allowed in inhomogeneous computations.\bigskip
	...
	\itemtt[PullingTriangulation] ditto.\bigskip
	
	\itemtt[Incidence] computes the incidence of extreme rays and facets.
	
	\itemtt[DualIncidence] the transpose of \verb|Incidence|.\bigskip
	
	\itemtt[IsEmptySemiopen] checks whether a semiopen polyhedron is empty.\bigskip
	...
	\itemtt[IsReesPrimary]: for the input type \verb|rees_algebra|, is the monomial ideal primary to the irrelevant maximal ideal?\bigskip
...
\begin{itemize}
	...
	\itemtt[KeepOrder, -k]  forces Normaliz to insert the generators (for generator input) or the inequalities (for constraint input) in the input order.
\end{itemize}
...
\begin{itemize}
	...
	\itemtt [Approximate, -r], approximation of rational polytopes for lattice point computation.\bigskip
...
\begin{itemize}
	...
	\itemtt[{-}{-}ignore, -i] This option disables all options in the input file.\bigskip
...
Via the option \ttt{WritePreComp} Normaliz can write an input file with precomputed data that can then be read by further computations. It has the suffix
\begin{itemize}
