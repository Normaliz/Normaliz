...
if [[ $BUILDSYSTEM == *static* ]]; then
    ...
    # libtool's -all-static only works for libraries that provide .la
    # files so fake the ones that are missing:
    for lib in arb flint cocoa antic; do
      echo '# Generated by libtool (GNU libtool) 2.4.6' > ${PREFIX}/lib/lib${lib}.la
