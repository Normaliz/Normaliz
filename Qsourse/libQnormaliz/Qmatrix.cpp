...
template<typename Integer>
vector<Integer> Matrix<Integer>::simplify_rows() {
    vector<Integer> g(nr);
    for (size_t i = 0; i <nr; i++) {
...
// IN THIS FIELD CASE IT MAKES THE CORNER ELEMENTS tTO 1
    ...
    for(size_t row=0;row<nr;++row){
        ...
        if(col==nc)
            continue;
        if(elem[row][col]<0)
            v_scalar_division<Integer>(elem[row],elem[row][col]); // !!!!!!!!!!!!!!!!!!
...
vector<Integer> Matrix<Integer>::solve_rectangular(const vector<Integer>& v, Integer& denom) const {
    ...
    Integer total_gcd = 1; // libnormaliz::gcd(denom,v_gcd(Linear_Form)); // extract the gcd of denom and solution
    denom/=total_gcd;
