...
void Output<Integer>::write_tri() const{
    if (tri==true) {
        ...
        const vector< pair<vector<libQnormaliz::key_t>,Integer> >& Tri = Result->getTriangulation();
        typename vector< pair<vector<libQnormaliz::key_t>,Integer> >::const_iterator tit = Tri.begin();        
