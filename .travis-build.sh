...
case $BUILDSYSTEM in
    *-scip*)
	...
	if test ! -f $SCIP_BUILD_DIR/scipoptsuite-$SCIPOPTSUITE_VERSION/.completed_build ; then
	    ...
	    wget --post-data
'tname=normaliz-travis-ci-build&license=academic&email=normaliz@uos.de&country=DE'
"http://scip.zib.de/download.php?fname=scipoptsuite-$SCIPOPTSUITE_VERSION.tgz"
--output-document=scipoptsuite-$SCIPOPTSUITE_VERSION.tgz
